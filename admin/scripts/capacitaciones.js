...
function guardaryeditar(e) {
  ...
  $.ajax({
    url: "../ajax/Ccapacitaciones.php?op=guardaryeditar",
    type: "POST",
    data: formData,
    contentType: false,
    processData: false,
...
function listar() {
  tabla = $("#tbllistado_carousel").dataTable({
    ...
    "ajax": {
      url: '../ajax/Ccapacitaciones.php?op=listar',
      type: "get",
      dataType: "json",
      error: function (e) {
...
function mostrar(idcapacitacion) {
  $.post("../ajax/Ccapacitaciones.php?op=mostrar", {
    ...
    if (data.foto == "") {
      ...
      $("#foto_i").attr("src", "../multimedia/capacitaciones/" + data.foto);
    }
  })
}

function desactivar(idcapacitacion) {
  swal({
    ...
    function (isConfirm) {
      if (isConfirm) {
        $.post("../ajax/Ccapacitaciones.php?op=desactivar", {
          idcapacitacion: idcapacitacion
        }, function (datos) {
          if (datos == 1) {
            swal({
...
function activar(idcapacitacion) {
  swal({
    ...
    function (isConfirm) {
      if (isConfirm) {
        $.post("../ajax/Ccapacitaciones.php?op=activar", {
          idcapacitacion: idcapacitacion
        }, function (datos) {
          if (datos == 1) {
            swal({
...
function eliminar(idcapacitacion) {
  swal({
    ...
    function (isConfirm) {
      if (isConfirm) {
        $.post("../ajax/Ccapacitaciones.php?op=eliminar", {
          idcapacitacion: idcapacitacion
        }, function (datos) {
          if (datos == 1) {
            swal({
