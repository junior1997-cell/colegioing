...
function guardaryeditar(e) {
  ...
  $.ajax({
    url: "../ajax/CGaleria.php?op=guardaryeditar",
    type: "POST",
    data: formData,
    contentType: false,
    processData: false,
...
function listar() {
    ...
    "ajax": {
			url: '../ajax/CGaleria.php?op=listar',
			type: "get",
			dataType: "json",
			error: function (e) {
...
function mostrar(idgaleria) {
  $.post("../ajax/CGaleria.php?op=mostrar", {
    idgaleria: idgaleria
    ...
    if (data.foto == "") {
      ...
      $("#foto_i").attr("src", "../multimedia/galeria/" + data.foto);
    }
...
function desactivar(idgaleria) {
	swal({
		...
		function (isConfirm) {
			if (isConfirm) {
				$.post("../ajax/CGaleria.php?op=desactivar", {
					idgaleria: idgaleria
				}, function (datos) {
          if (datos == 1) {
            swal({
...
function activar(idgaleria) {
	swal({
		...
		function (isConfirm) {
			if (isConfirm) {
				$.post("../ajax/CGaleria.php?op=activar", {
					idgaleria: idgaleria
				}, function (datos) {
          if (datos == 1) {
            swal({
...
function eliminar(idgaleria) {
	swal({
		...
		function (isConfirm) {
			if (isConfirm) {
				$.post("../ajax/CGaleria.php?op=eliminar", {
					idgaleria: idgaleria
				}, function (datos) {
          if (datos == 1) {
            swal({
