...
function mostrar(idcertificadoH) {
  $.post("../ajax/Ccertihabilidad.php?op=mostrar", {
...
function desactivar(idcertificadoH) {
  swal({
    ...
    function (isConfirm) {
      if (isConfirm) {
        $.post("../ajax/Ccertihabilidad.php?op=desactivar", {
          idcertificadoH: idcertificadoH
        }, function (datos) {
          if (datos == 1) {
            swal({
...
function activar(idcertificadoH) {
  swal({
    ...
    function (isConfirm) {
      if (isConfirm) {
        $.post("../ajax/Ccertihabilidad.php?op=activar", {
          idcertificadoH: idcertificadoH
        }, function (datos) {
          if (datos == 1) {
            swal({
...
function eliminar(idcertificadoH) {
  swal({
    ...
    function (isConfirm) {
      if (isConfirm) {
        $.post("../ajax/Ccertihabilidad.php?op=eliminar", {
          idcertificadoH: idcertificadoH
        }, function (datos) {
          if (datos == 1) {
            swal({
 ...
 * costos
 */
function ver_costo(idcertificadoH) {
  $(document).ready(function () {

    $("#ver_costo").modal('show');
  });

  $("#tbllistado_costos").dataTable({
    "aProcessing": true,
    "aServerSide": true,
    dom: 'Bfrtip',
    "language": {
    ...
    "ajax": {
      url: '../ajax/Ccertihabilidad.php?op=listarcosto&id=idcertificadoH',
      type: "get",
      dataType: "json",
      error: function (e) {
        console.log(e.responseText);
